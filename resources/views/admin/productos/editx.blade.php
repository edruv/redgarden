				...
				<form action="{{route('productos.update',$product->id)}}" method="post">
						...
						<div class="col-md form-group">
							...
							<select name="categoria" id="categoria" class="custom-select">
								...
								@foreach ($categParent as $parent)
									<option @if ($parent->id == $product->categoria) selected @endif value="{{$parent->id}}">{{ ucfirst($parent->nombre) }}</option>
								@endforeach
							</select>
						</div>
					</div>
					<div class="row text-center">
						<div class="col-md form-group">
							...
							<input type="text" name="precio" id="precio" class="form-control" value="{{ $product->precio }}" required>
						</div>
						<div class="col-md form-group">
							<label for="descuento">Descuento</label>
							<input type="text" name="descuento" id="descuento" class="form-control" value="{{ $product->descuento }}" placeholder="%">
						</div>
					</div>
